...
export function UpdatePrompt() {
  ...
  return (
    <div className="safe-bottom pointer-events-none fixed inset-x-0 bottom-0 z-[60] flex justify-center px-4 pb-24">
      <div className="pointer-events-auto w-full max-w-md rounded-2xl bg-slate-800 p-4 shadow-2xl shadow-black/40 ring-1 ring-white/10">
        {needRefresh ? (
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-slate-100">新しいバージョンがあります</p>
              <p className="text-xs text-slate-400">最新の内容に更新できます。</p>
            </div>
            <button
              type="button"
              onClick={close}
              className="rounded-xl bg-slate-700/60 px-3 py-2 text-sm font-semibold text-slate-200"
            >
              後で
            </button>
            <button
              ...
              className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-bold text-slate-900"
            >
              更新
            </button>
          </div>
